...
const NOT_TURN = function () {
  return TURN === GUTI_COLOR.PLAYER1 ? GUTI_COLOR.PLAYER2 : GUTI_COLOR.PLAYER1;
};

const SET_MY_GUTI_COLOR = function () {
  if(MY_PLAYER_ID == 1){
    MY_GUTI_COLOR = GUTI_COLOR.PLAYER1;
  } else {
    MY_GUTI_COLOR = GUTI_COLOR.PLAYER2;
  }
  console.log("Setting guti color", MY_GUTI_COLOR);
}

function updateTurn() {
  let turnElem = document.getElementById("turn");
  turnElem.style.color =
    TURN === GUTI_COLOR.PLAYER1 ? GUTI_COLOR.PLAYER1 : GUTI_COLOR.PLAYER2;
  turnElem.className =
    TURN === GUTI_COLOR.PLAYER1 ? "animateToRight" : "animateToLeft";
  turnElem.style.top = TURN === GUTI_COLOR.PLAYER1 ? "75vh" : "10vh";
  turnElem.innerText =
    TURN === GUTI_COLOR.PLAYER1 ? "Greens turn" : "Pinks turn";
}
...
class GutiManager {
    ...
    for (let guti of this.getGutiPositions()) {
      ...
      if (guti.color === TURN && TURN === MY_GUTI_COLOR) {
        ...
        circle.addEventListener("click", () => {
          ...
          GutiManager.update = false;
          this.draw();
          console.log("sending nextTurn: " + TURN);
        });
      }
  ...
  killHandler(src, dest) {
		const diff = Math.abs(src - dest);
		const min = Math.min(src, dest);
      ...
      if(this.score.pink == 10){
        document.getElementById("you").innerText = "Pink won";
      } else if(this.score.green == 10){
        document.getElementById("you").innerText = "Green won";
      }
		}
	}
}
